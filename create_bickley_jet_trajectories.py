...
tau = 10 * 86400 #days in seconds
dt_output = 86400/10 #daily output
dt_int = 60 #60 seconds integration time step
n_it = int(dt_output / dt_int) #index of output

#discretization
t = np.arange(0, tau + dt_output, dt_int) 
x_range = np.linspace(0,1,100) * np.pi * r0 #in km
y_range = np.linspace(-2.5,2.5,50) * 1000 #in km
...
np.savez("bickley_jet_trajectories_tau" + str(tau) + "_np_" + str(len(X0)), drifter_longitudes = trajectories_x, 
