...
def SST_clutering():
    ...
    indices = np.argwhere(drifter_data.trajectory_lengths>365)[:,0]
    
    
    
    
    #REDUCE DRIFTERS then to the year of data!!!
    ...
    sst_hist_anomaly = np.array([np.histogram(SST_anomaly[i], bins=nbins, density=True, 
                                              range=(SST_min_anomaly, SST_max_anomaly))[0] for i in range(len(SST_anomaly))])
